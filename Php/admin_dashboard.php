...
<head>
    ...
    <title>Schecter Guitars - Admin</title>
...
if ($res) { $row = mysqli_fetch_assoc($res); echo "Total Users: " . $row["total"] . "<br>"; }

$stmt = "SELECT COUNT(*) as total FROM `orders`";
$res  = mysqli_query($conn, $stmt);
if ($res) { $row = mysqli_fetch_assoc($res); echo "Total Orders: " . $row["total"] . "<br>"; }

$stmt = "SELECT COUNT(*) as total FROM `contact_messages`";
$res  = mysqli_query($conn, $stmt);
if ($res) { $row = mysqli_fetch_assoc($res); echo "Contact Messages: " . $row["total"] . "<br>"; }
?>
